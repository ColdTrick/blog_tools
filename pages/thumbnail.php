...
if (!isset($_GET['guid']) || !isset($_GET['blog_guid'])) {
...
$guid = (int) $_GET['guid'];
$blog_guid = (int) $_GET['blog_guid'];

// If is the same ETag, content didn't changed.
$etag = md5($icontime . $size . $blog_guid . $guid);
...
if (!$filecontents && $size !== 'medium') {
	$filename = "{$entity_path}blogs/{$blog_guid}medium.jpg";
	$filecontents = @file_get_contents($filename);
}
